  ...
  localStorage.setItem('playerGameCode',gameCode)
    ...
    const getPlayers=async()=>{
      try {
        ...
        if(res.data.playerStatus)navigate(`/game/${gameCode}`)
    ...
    useEffect(()=>{
      const getPlayers=async()=>{
        try {
          ...
          const res=await axios.post(`${BACKENDURL}/game/players`,{token:token,gameCode:gameCode})
          setPlayers(res.data.players)
          if(res.data.gameStatus==='In Play')navigate(`/game/${gameCode}`)
          setKick(res.data.kick)
          setRoomName(res.data.RoomName)
          setCanKick(res.data.canKick)
          setPlayerSize(res.data.playerSize)
    ...
    const StartGame=async()=>{
      ...
      try {
        let token = localStorage.getItem ('gameUserToken');
        await axios.post(`${BACKENDURL}/game/start`,{token:token,gameCode:gameCode})
        setStarting(false)
        navigate(`/game/${gameCode}`)
    ...
    const CloseGame=async()=>{
      ...
      try {
        let token = localStorage.getItem ('gameUserToken');
        await axios.post(`${BACKENDURL}/game/close`,{token:token,gameCode:gameCode})
        setClosing(false)
        navigate('/home')
  ...
  return (
    <>{!roomError ?
      <div className={styles.cont}>
        ...
        <button className={styles.closebtn} disabled={closing||kick!==canKick} onClick={CloseGame}>{closing?'Closing':"Delete game!"}</button>
